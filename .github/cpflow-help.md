...
example `v5.3.0`, so `CPFLOW_VERSION` can validate against a semantic release.
...
override goes with wrappers pinned to the `v5.3.x` tag.
...
bin/pin-cpflow-github-ref --checkout-ref <40-character-control-plane-flow-commit-sha> v5.3.0
